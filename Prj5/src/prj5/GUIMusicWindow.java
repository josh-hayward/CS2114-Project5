...
public class GUIMusicWindow {
     ...
     */
    public void drawGlyphs(CategoryEnum category) {
        ...
        {
            case MAJOR:
                percentages = Input.representMajor(students, songs);
                break;
            case REGION:
                percentages = Input.representRegion(students, songs);
                break;
            case HOBBY:
                percentages = Input.representHobbies(students, songs);
                break;
            default:
                percentages = Input.representHobbies(students, songs);
                break;
            ...
            for (int j = 0; j < 3; j++) {
                ...
                if (index < songs.size()) {
                    ...
                    GUIGlyph glyph = new GUIGlyph(window, xcor, ycor, songs
                        .getSong(index), percentages.getNodeAt(index).getData());
                    glyph.draw();
                }
            }
        }
